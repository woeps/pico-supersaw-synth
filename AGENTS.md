...
- **Dual-Core Safety**: Core 1 handles MIDI UART **polling/parsing** and renders voices 2–3 into `core1ScratchBuf`. Core 0 **dispatches MIDI events** (`noteOn`/`noteOff`/`setCC`), renders voices 0–1 into `core0ScratchBuf`, merges both `int32_t` scratch buffers with ÷2 normalization, applies resonance-compensated filter and crossfade chorus, and writes the final mix to the I2S audio buffer. Both scratch buffers use `int32_t` to preserve full dynamic range — clamping to `int16_t` is deferred until after the merge. Use `volatile` modifiers and hardware spinlocks (`spin_lock_t`) for cross-core shared variables.
- **Gain Staging**: The SVFilter applies resonance gain compensation (`resoCompGain` in Q14) to attenuate input by `1/Q` when Q > 1, preventing the resonance peak from clipping. The StereoChorus uses a crossfade wet/dry mix (not additive) so total gain never exceeds unity.
