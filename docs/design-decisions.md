...
All arithmetic uses Q14 fixed-point or combinations thereof. Because the difference equation requires computing a division of state variables, the high-pass calculation (`hp_num << 13 / D_half`) efficiently utilizes the RP2040's native 8-cycle divider `hw_divider_quotient_s32`. This keeps the computational overhead minimal, replacing what would otherwise be a slow software division. State variables are clamped to ±32767 after each SVF iteration to bound the worst-case arithmetic safely within `int32_t` range on the M0+ core. The cutoff coefficient is stored in a 128-entry flash lookup table to avoid runtime trigonometry.
