...
| 85 | Pitch-Bend Range | 0–127 → 0–24 semitones (default ~4 st) |

## Pitch Bend

Pitch-bend messages (status `0xE0`, 14-bit value 0–16383, centre 8192) are parsed by Core 1, packed into the MIDI event FIFO, and dispatched on Core 0. The bend amount is applied as a **global frequency-ratio multiplier** (Q16.16) to the base phase increment in `updateDetuneForVoice`, so all 7 oscillators in every active voice track the bend identically on both cores.

The range is configurable at runtime via **CC 85** (`CC_PITCHBEND_RANGE`). The mapping is linear: `bendRange = value × 24 / 127` (0–24 semitones). The default boot value is **4 semitones** (raw CC ≈ 21).

Because the RP2040 lacks a hardware FPU, the exponential ratio `2^(semitoneOffset/12)` is precomputed into a **flash-resident lookup table** (`pitchBendRatioTable`, 1537 entries, Q16.16) covering ±24 semitones at 32 steps per semitone (~3.1 cent resolution). At runtime, the bend value and current range are combined with integer math to compute a table index — no floating-point or transcendental functions are executed on the audio path.

The table is generated by `scripts/generate_pitchbend_table.py` and lives in `src/synth/pitchbend_table.h`.
...
**Reduced critical section:** `cacheAcquire()` originally held the interrupt-disabling `cacheLock` for the entire 4 KB flash `memcpy`, stalling the other core for the full flash-read latency. The slow flash read is now staged through a dedicated `cacheScratch[WAVETABLE_SIZE]` member buffer (~4 KB SRAM) performed with **no lock held**. The lock is only re-acquired for the fast SRAM→SRAM copy into the chosen cache slot, after a re-check confirms another path did not insert the same band in the meantime. This is safe because `cacheAcquire()` is only ever called from `noteOn()` (Core 0), so the single scratch buffer has no concurrent writers. The critical section shrinks from a flash-latency copy to a fast SRAM copy.
...
The per-sample division (`hp = hp_num / D`) uses a **precomputed reciprocal multiply** instead of the hardware divider. `invD = round(2²⁸ / D)` is computed once in `setCutoff()` / `setResonance()`, then the per-sample division becomes a 64-bit multiply-and-shift: `hp = (int64_t)hp_num * invD >> 14`. This is both faster (~5 cycles vs. 8 for `hw_divider_quotient_s32`) and avoids the 32-bit numerator overflow that would occur with Q28-scaled `hp_num`. The cutoff coefficient is stored in a 128-entry flash lookup table to avoid runtime trigonometry.

**32-bit feedback multiply:** The feedback terms in the `hp_num` numerator (`R × s1` and `g × s1`) previously used `1LL * R * s1 >> 14`, forcing a slow 64-bit software multiply every sample on the FPU-less M0+. Because the Q28 state `s1` is shifted right by 14 *before* multiplying by the Q14 coefficient (`s1_q14 = s1 >> 14`), the products `R × s1_q14` and `g × s1_q14` stay in 32-bit while preserving the same Q28 net scaling. The worst case is bounded: `s1` max = `STATE_MAX` (2²⁸) → `s1 >> 14 ≈ 16384`; `R`/`g` max ≈ 35631 → product ≈ 5.8 × 10⁸, well within `int32_t`. Only the low 14 bits of state are truncated, which is inaudible. The `invD` reciprocal multiply and the `g × hp` / `g × bp` integrator products still require 64-bit and are left unchanged. The identical expressions in `init()`/`setCutoff()`/`setResonance()` run only on parameter change (not in the audio loop) and are left as-is.
...
The allpass interpolation coefficient `a = (256 − frac) × 256 / (256 + frac)` is read from a precomputed 256-entry lookup table (`allpassCoeffTable`, generated by `scripts/generate_chorus_table.py`) rather than computed via integer division at runtime. This saves ~100–140 cycles per stereo sample pair on the Cortex-M0+ (which lacks a hardware divider for general-purpose division).
...
The flash erase (~50 ms) runs inside `save_and_disable_interrupts()`, which freezes the I2S DMA completion IRQ. Because `pico_audio_i2s` only loads the next buffer from inside that IRQ handler, **no queued buffer can advance while flash is being written** — pre-filling more buffers cannot help, it merely delays the freeze by a single in-flight buffer (~5.8 ms). When the DMA stalls, the PIO underruns and holds the *last sample* on the line; if that sample is non-zero the result is an audible DC click/buzz.

The fix (in `src/main.cpp`) makes the frozen in-flight sample zero and reaches it smoothly:

1. **Fade out** to silence over `FADE_BUFFERS` (3) buffers via `renderWithFade()`, ramping a Q15 output gain per-sample (no zipper noise) while the DMA IRQ is still active.
2. **Flush zero buffers** with blocking takes (`flushSilenceInFlight()`, `AUDIO_BUFFER_COUNT + 1` iterations) so a zeroed buffer is guaranteed to be the one in flight when flash freezes.
3. **Write flash** — the PIO now underruns on a zero sample → clean silence, no click.
4. **Fade in** back to full over `FADE_BUFFERS` buffers.

The total deliberate dip (~110 ms: fade-out + flush + erase + fade-in) is click-free and acceptable for a manual 5 s-hold save. The fade only scales rendered output, so voice/envelope state is preserved and held notes resume correctly.

## Build Optimization

The `CMakeLists.txt` defaults `CMAKE_BUILD_TYPE` to `Release` when unset and applies `-O3` to the `supersaw_midi_synth` target. Without an explicit build type the toolchain compiles the fixed-point DSP unoptimized, wasting cycles the audio loop cannot spare on the 125 MHz M0+. `-O3` enables aggressive inlining and loop optimization for the per-sample render path. The build type can still be overridden on the command line (e.g. `-DCMAKE_BUILD_TYPE=Debug`) since the default only applies when the variable is not already set.
