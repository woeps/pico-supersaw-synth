...
Pitch-bend wheel messages (status `0xE0`, 14-bit value 0–16383, centre 8192) are parsed and dispatched by the MIDI input layer. The bend amount is applied as a **global frequency-ratio multiplier** to the base phase increment in `updateDetuneForVoice`, so all 7 oscillators in every active voice track the bend identically on both cores.

### Bend Range

The bend range is configurable at runtime via **CC 85** (`CC_PITCHBEND_RANGE`):

```
bendRange = value × 24 / 127   (0–24 semitones)
```

The default boot value is **4 semitones** (raw CC ≈ 21).

### Table-Based Ratio

Because the RP2040 has no hardware FPU, the exponential ratio `2^(semitoneOffset/12)` is precomputed into a **flash-resident lookup table** (`pitchBendRatioTable`, 1537 entries, Q16.16) covering ±24 semitones at 32 steps per semitone (~3.1 cent resolution). At runtime, the 14-bit bend value and current range are combined with integer math to compute a table index:

```
idx = ((bend14 - 8192) × bendRange × 32) / 8192 + CENTER_INDEX
```

No floating-point or transcendental functions run on the audio path. The table is generated by `scripts/generate_pitchbend_table.py`.
